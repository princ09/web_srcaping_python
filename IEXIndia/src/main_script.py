...
def write_in_csv(soup):
    data_table = soup.find('table', {'cols': 18})
    data_table_trs = data_table.findAll('tr', {'valign': 'top'})
    with open('data.csv', mode='w') as schedule_file:
        data_writer = csv.writer(schedule_file, delimiter=',', quotechar='"', quoting=csv.QUOTE_MINIMAL)
        for data_table_tr in data_table_trs:
            temp = []
            data_table_tds = data_table_tr.findAll('div')
            for data_table_td in data_table_tds:
                temp.append(data_table_td.text)
...
def get_table_data_by_date(from_date, to_date):
    payload = {
        '__EVENTTARGET': '',
        '__EVENTARGUMENT': '',
        '__LASTFOCUS': '',
    ...
    view_state = re.search(r'__VIEWSTATE"\svalue="(.*)"', response.text)
    payload['__VIEWSTATE'] = str(view_state.group(1))

    event_validation = re.search(r'__EVENTVALIDATION"\svalue="(.*)"', response.text)
    payload['__EVENTVALIDATION'] = str(event_validation.group(1))
    responsee = requests.post(url, headers=header, data=payload)
    soup = BeautifulSoup(responsee.content, 'html.parser')
    write_in_csv(soup)

get_table_data_by_date('02/10/2020', '08/10/2020')
